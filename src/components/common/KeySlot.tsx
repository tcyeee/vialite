...
interface Props {
  ...
  emptyLabel?: string;
  /** The whole keycode 复制 should copy, when this slot only edits part of it —
   *  see {@link ../keymap/picker/KeycodeCascadeSelector}'s `copyValue`. Left
   *  unset, 复制 copies this slot's own `qmkId`. */
  copyId?: string;
  /** Pop the cascade selector as soon as this slot appears — for a slot that is
   *  only mounted because the user just asked to set its key. */
  autoOpen?: boolean;
}
