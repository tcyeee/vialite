 ...
 */
function TapDancePreviewCard({ index, entry, onSave, onDelete }: PreviewCardProps) {
  const { t } = useI18n();
  ...
  return (
    <div className="group/card relative my-2 w-80" style={{ perspective: "1200px" }}>
      {editable && !flipped && (
        <div className="absolute -top-3 left-1/2 z-10 flex -translate-x-1/2 gap-1 rounded-full bg-neutral-900 px-2 py-1 opacity-0 shadow-lg transition-opacity group-hover/card:opacity-100">
          <button
            type="button"
              ...
              {PREVIEW_STATES.map(({ labelKey, field }) => (
                <div key={field}>
                  ...
                  <div className="whitespace-pre-line">
                    {kcLabel(entry[field] as string) || (entry[field] as string)}
                  </div>
                </div>
              ))}
            </div>
        ...
        {editable && (
          ...
          >
            <div className="card-body">
              ...
              <label className="fieldset-label text-neutral-600">{t("tapDanceTappingTerm")}</label>
              <input
                type="number"
                className="input mb-3 w-28"
                min={0}
                max={10000}
              ...
              <div className="grid grid-cols-2 gap-3">
                {PREVIEW_STATES.map(({ labelKey, field }) => (
                  <div key={field}>
                    <label className="fieldset-label text-neutral-600">{t(labelKey)}</label>
                    <KeySlot
                      qmkId={entry[field] as string}
                      onChange={(id) => onSave?.({ [field]: id } as Partial<TapDanceEntry>)}
                      className="btn btn-outline min-h-12 w-full flex-wrap py-1 text-xs whitespace-pre-line"
                    />
                  </div>
                ))}
              </div>
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap gap-4">
        {keyboard.tapDanceEntries.map((e, i) => (
          <TapDancePreviewCard
            key={i}
            index={i}
            entry={e}
